<template>
  <b-modal centered  hide-header hide-footer  body-class="x-modal" id="gi-text-modal">
      <header class="x-modal__header">
        <span class="title">Update <span>{{formatName(field.name).toLowerCase()}}</span></span>
      </header>
      <main>
          ...
          <form @submit.prevent="onUpdate">
            <ValidationProvider v-slot='{errors}' rules='required'>
              <div class="form-group">
                ...
                <b-input class="form-control" v-model='innerField.value' :placeholder="`New ${formatName(field.name)}`"/>
              </div>
            ...
            <footer class="x-modal__footer">
              ...
              <button type="cancel" @click="$bvModal.hide('gi-text-modal')" class="btn btn-action">Cancel</button>
            </footer>
          </form>
        </ValidationObserver>
      </main>
...
export default class GITextModal extends Vue{
  ...
  async onUpdate(){
    ...
    //@ts-ignore
    this.$bvModal.hide('gi-text-modal')
  }
  formatName(value: string){
    if(!value) return '';
    const res = value.split('_').join(" ")
