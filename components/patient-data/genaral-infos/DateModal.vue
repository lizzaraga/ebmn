          ...
          <form @submit.prevent>
            <ValidationProvider v-slot='{errors}' rules='required'>
              <div class="form-group">
                ...
                <b-input type="date" class="form-control" v-model='innerField.value' :placeholder="`New ${formatName(field.name)}`"/>
              </div>
            ...
            <footer class="x-modal__footer">
              ...
              <button type="cancel" @click="$bvModal.hide('gi-date-modal')" class="btn btn-action">Cancel</button>
            </footer>
          </form>
        </ValidationObserver>
      </main>
...
export default class GIDateModal extends Vue{
  ...
  async onUpdate(){
    ...
    //@ts-ignore
    this.$bvModal.hide('gi-date-modal')
  }
  formatName(value: string){
    if(!value) return '';
    const res = value.split('_').join(" ")
