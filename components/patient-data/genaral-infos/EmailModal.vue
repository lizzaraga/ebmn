<template>
  <b-modal centered  hide-header hide-footer  body-class="x-modal" id="gi-email-modal">
          ...
          <form @submit.prevent>
            <ValidationProvider v-slot='{errors}' rules='required|email'>
              <div class="form-group">
                ...
                <b-input type="email" class="form-control" v-model='innerField.value' :placeholder="`New ${formatName(field.name)}`"/>
              </div>
...
export default class GIEmailModal extends Vue{
  ...
  async onUpdate(){
    ...
    //@ts-ignore
    this.$bvModal.hide('gi-tel-modal')
  }
  formatName(value: string){
    if(!value) return '';
    const res = value.split('_').join(" ")
