<template>
  <b-modal size="lg" hide-header hide-footer  body-class="x-modal" id="create-ekg-modal">
        ...
        <div class="row">
          <b-form-group class="col-6 position-relative"  label="Description"> 
            <b-form-input  debounce="100" @focus="showSearchResults"
             v-model="ekg.description"  name="description" id="description"></b-form-input>
          ...
          <b-form-group class="col-6"  label="Loinc Code"> 
            <b-form-input :disabled="ekg.loincCode.length == 0" v-model="ekg.loincCode" 
             name="loinc_code" id="loinc_code"></b-form-input>
          </b-form-group>
        </div>
        <div class="row">
          <b-form-group class="col-6"  label="Ordered At"> 
            <b-form-select :value="null" :disabled="ekg.hospitals.length <= 0" :options="ekg.hospitals" name="ordered_at" id="ordered_at"></b-form-select>
          </b-form-group>
          <b-form-group class="col-6" label="Done At"> 
            <b-form-select :value='null' :disabled="ekg.hi.length <= 0" :options="ekg.hi" name="done_at" id="done_at"></b-form-select>
          </b-form-group>
        </div>
        <div class="row">
          <b-form-group class="col-12" label="Related Problems"> 
            <b-form-select v-model='selectedPbs' name="related_problems" :disabled="ekg.problems.length <= 0" :options="ekg.problems" multiple ></b-form-select>
...
@Component
export default class CreateEkgModal extends Vue{
  ...
  doStartCreate(){
    const form = document.querySelector("#create-ekg-form")
    // @ts-ignore
  ...
  onSelectLab(lab: ILabInstitute){
    ...
    // @ts-ignore
    this.ekg = {...this.ekg, ...{loincCode: lab.lab_loinc_code, description: lab.lab_description}}
  ...
  async created(){
    ...
    try {
      ...
      // @ts-ignore
      this.ekg = {...this.ekg, ...{problems: pbs}}
    } catch (error) {
